...
void resetVelocity();
void publish_emotion_face(ros::Publisher emotion_face_pub_,
                          Emotion current_emotion);
...
int main(int argc, char** argv) {
  ...
  ros::Publisher vel_pub_ = nh_.advertise < geometry_msgs::Twist
      > ("cmd_vel", 5);
  ros::Publisher emotion_info_pub_ = nh_.advertise < std_msgs::String
      > ("emotion/info", 5);
  ros::Publisher emotion_face_pub_ = nh_.advertise < std_msgs::String
      > ("emotion/face", 5);
  ros::Subscriber segbot_teleop_sub_ = nh_.subscribe("cmd_vel", 5,
                                                     segbot_teleop_handler);
  ...
  diff_percentage = 0.0;
  desire_level = 0.0;
  happy_threshold = static_cast<float>(rand())
      / (static_cast<float>(RAND_MAX / (1.1 - 0.1)));  // 0.1 to 1.1, inclusive
  sad_threshold = (static_cast<float>(rand())
      / (static_cast<float>(RAND_MAX / (1.1 - 0.1)))) * -1;  // -0.1 to -1.1, inclusive
  angry_threshold = sad_threshold - 1.0
      - (static_cast<float>(rand())
          / (static_cast<float>(RAND_MAX / (1.1 - 0.1))));  // always less than (sad_threshold - 1.0)
...
bool checkResponse() {
  if (current_movement == MOVE_FORWARD
      && (velocity.linear.x <= 0.0 || velocity.angular.z != 0.0)) {
    num_no++;
    return isCommandFollowed = false;
  } else if (current_movement == TURN_LEFT
      && (velocity.linear.x != 0.0 || velocity.angular.z <= 0.0)) {
    num_no++;
    return isCommandFollowed = false;
  } else if (current_movement == TURN_RIGHT
      && (velocity.linear.x != 0.0 || velocity.angular.z >= 0.0)) {
    num_no++;
    return isCommandFollowed = false;
  } else if (current_movement == MOVE_BACKWARD
      && (velocity.linear.x >= 0.0 || velocity.angular.z != 0.0)) {
    num_no++;
    return isCommandFollowed = false;
  } else {
...
Emotion determineMoodLevel(bool isCommandFollowed) {
  ...
  diff_percentage = (static_cast<float>(num_yes)
      / static_cast<float>(total_num_commands))
      - (static_cast<float>(num_no) / static_cast<float>(total_num_commands));
